...
int main(int argc, char** argv)
{
    ...
    int count = 0;
     ...
     ur5::joint_state q;
     kul::pose_t fr_forearm__fr_base;
    ...
    while( ! data.eof() )
    {
        ...
        ur5::fk2(mc, q, fr_forearm__fr_base);

        Rdiff = kul::orientationDistance(
                    kul::eg_get_rotation(fr_forearm__fr_base),
                    kul::eg_get_rotation(given_1));
        err_ori += Rdiff.angle;
        err_pos += (kul::eg_get_position(fr_forearm__fr_base-given_1)).norm();
        count++;
    }
